  ...
  >
    <template v-slot="{ result: { data } }">
      <n-layout style="height: 100%">
        ...
        <n-layout-content content-style="padding: 24px;">
          <n-card>
            ...
            <n-grid x-gap="16" y-gap="16" :cols="4" v-if="data">
              <n-grid-item v-for="caseItem in data.getCases">
                <n-config-provider :theme="caseItem.theme">
                  <n-card hoverable>
                    <template #header>
                      <n-h2 style="cursor: pointer">
                        {{ caseItem.title }}
                      </n-h2>
                    </template>
                    <n-h3>
      ...
      <n-modal v-model:show="showLoginModal" style="width: 400px" preset="card">
        ...
        <template #footer>
          <n-space vertical>
            <n-button strong secondary type="primary" style="width: 100%" @click="login"> 登录 </n-button>
            <n-button quaternary type="info" style="width: 100%">没有账号？注册</n-button>
          </n-space>
        </template>
      </n-modal>
    </template>
